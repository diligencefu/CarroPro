...
class LNCarEnhanceCell: LNCarCell {
    ...
    lazy var enhanceTitle: UILabel = {
        ...
        enhanceTitle.font = UIFont.boldSystemFont(ofSize: 17)
    ...
    public override var resource: LNCarInfoModel? {
        ...
        didSet {
            ...
            let eWidth = kNormalWidth - enhanceImage.ln_right - kLeftSpace
            enhanceTitle.frame = CGRect.init(x: enhanceImage.ln_right + 12, y: kTop, width: eWidth, height: enhanceImage.ln_height/4)
            enhanceTitle.text = "Concierge Service"
            
            let dHeight = enhanceImage.ln_height/4 * 3 - 3
            enhanceDesc.frame = CGRect.init(x: enhanceTitle.ln_x, y: enhanceTitle.ln_bottom + 3, width: eWidth, height: dHeight)
